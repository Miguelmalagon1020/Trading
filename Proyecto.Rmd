---
title: "Proyecto_Trading"
author: "Bula-Malagón-Serrano"
date: "`r Sys.Date()`"
output: html_document
---

# Introducción 




## Parametrización de la estrategia


```{r}
# Medias moviles doble crossover method "Exponenciales" 
ticket <- "MSFT"
from <- "2020-01-01"
periodicidad <- "daily"   #"weekly", "monthly"
EMA1 <- 20
EMA2 <- 50

# MACD & Signal Line

rapida <- 12 
lenta <- 26 
signo <- 9

#Oscilador RSI 
MM_RSI <- 14

# Graficos 

ventana=120 #relacionado con la periodicidad 

```



# Cotización del Microsoft

```{r,warning=FALSE}
library(quantmod)
library(ggplot2)
#install.packages("TTR")
library(TTR)
library(dygraphs)
library(dplyr)
library(tidyverse)
update.packages()
```

```{r}
activo <- getSymbols(Symbols = ticket,src = "yahoo", from =from, to=Sys.Date(),
                     periodicity = periodicidad, auto.assign = FALSE)
PRECIO <- activo[,4]
```


## Medias moviles exponenciales doble crossover method

```{r}
EMA <- function (price,n){
  ema <- c()
  ema[1:(n-1)] <- NA
  ema[n]<- mean(price[1:n])
  beta <- 2/(n+1)
  for (i in (n+1):length(price)){
    ema[i]<-beta * price[i] + 
      (1-beta) * ema[i-1]
  }
  ema <- reclass(ema,price)
  return(ema)
}

MA1 <- EMA(PRECIO,EMA1)
MA2 <- EMA(PRECIO,EMA2)


x <- cbind(activo[,c(1:4)],MA1,MA2)
cola <- tail(x, n=ventana)
grap<-dygraph(cola)
  dyCandlestick(grap)%>%
  dyRangeSelector()

```
```{r}
xy <- cbind(MA2,MA1)
dygraph(xy)%>%
  dyRangeSelector()

```


## MACD oscilador 

```{r}
MACD <- function (precio,Short,Large,sig){
  MACD <- na.omit(EMA(precio,Short) - EMA(precio,Large))
  signal <- EMA(MACD,sig)
  tabla <- cbind(MACD,signal)
  colnames(tabla) <- c("MACD","signal")
  return(tabla)
}


MACD_data <- MACD(precio = PRECIO,Short =rapida ,Large = lenta,sig = signo)

dygraph(MACD_data)%>%
  dyRangeSelector()


```

## Señales con la MEDIA MOVIL Y MACD

### Señal de compra

```{r}
siganls_buy <- function(ma1,ma2){
  
  signal_buy <- c()
  inicio <- (nrow(ma2)-nrow(filter(as.data.frame(ma2),V1!="NA")))
  ma2[inicio,1] <- 0
  for (i in (inicio+1):nrow(MA2)) {
    if ((ma1[i,1]>ma2[i,1])&&(ma1[i-1,1]<ma2[i-1,1])) {
      signal_buy[i] <- 1
    }else{
      signal_buy[i] <- 0
    }
  }
x <- cbind(ma1,ma2,signal_buy)
filtro <- filter(as.data.frame(x),signal_buy==1)
return(filtro)
}

siganls_buy(ma1 = MA1,ma2 = MA2)
```
### Señal de venta 

```{r}
siganls_sell<- function(ma1,ma2){
  
  signal_sell <- c()
  inicio <- (nrow(ma2)-nrow(filter(as.data.frame(ma2),V1!="NA")))
  ma2[inicio,1] <- 0
  for (i in (inicio+1):nrow(MA2)) {
    if ((ma1[i,1]<ma2[i,1])&&(ma1[i-1,1]>ma2[i-1,1])) {
      signal_sell[i] <- 1
    }else{
      signal_sell[i] <- 0
    }
  }
x <- cbind(ma1,ma2,signal_sell)
filtro <- filter(as.data.frame(x),signal_sell==1)
return(filtro)
}

siganls_sell(ma1 = MA1,ma2 = MA2)
```

## Confiormacion con MACD
### señal compra

```{r}
siganls_buy_MACD <- function(macd){
  
  signal_buy <- c()
  inicio <- (nrow(macd)-nrow(filter(as.data.frame(macd[,2]),"signal"!="NA")))
  macd[inicio,2] <- 0
  for (i in (inicio+1):nrow(macd)) {
    if ((macd[i,1]>macd[i,2])&&(macd[i-1,1]<macd[i-1,2])) {
      signal_buy[i] <- 1
    }else{
      signal_buy[i] <- 0
    }
  }
x <- cbind(macd,signal_buy)
filtro <- filter(as.data.frame(x),signal_buy==1)
return(filtro)
}

siganls_buy_MACD(macd = MACD_data)



Filtrico <- filter(as.data.frame(MACD_data$signal),"signal"!="NA")






```

# RSI oscilador 

```{r}
Sobrecompra <- 70
Sobreventa <- 30
myRSI <- function (PRECIO,n){
  N <- length(PRECIO)
  U <- rep(0,N)
  D <- rep(0,N)
  rsi <- rep(NA,N)
  Lprice <- Lag(PRECIO,1)
  for (i in 2:N){
    if (PRECIO[i]>=Lprice[i]){
      U[i] <- 1
    } else {
      D[i] <- 1
    }
    if (i>n){
      AvgUp <- mean(U[(i-n+1):i])
      AvgDn <- mean(D[(i-n+1):i])
      rsi[i] <- AvgUp/(AvgUp+AvgDn)*100 
      }
    }
  rsi <- reclass(rsi, PRECIO)
  return(rsi)
}

rsi <- myRSI(Cl(PRECIO), MM_RSI)
RSI <- cbind(rsi,Sobrecompra,Sobreventa)
#Gráfico
Graf_RSI <- dygraph(RSI, main= "RSI oscilator")%>%
  dyRangeSelector()
Graf_RSI
```
#Señales con RSI
```{r}
senal_rsi <- function(oscilador,S_venta,S_compra,inicio){
  senal <- c()
  INICIO <- inicio+1
  TILL <- nrow(oscilador)
  for (i in INICIO:TILL) {
      if((oscilador[i,1]<S_venta)&&(oscilador[i-1,1]>S_venta)){
      senal[i] <- 1
    }else 
      if((oscilador[i,1]>S_compra)&&(oscilador[i-1,1]<S_compra)){
      senal[i] <- -1}
    else{
      senal[i] <- 0
    }
  }
  z <- cbind(oscilador,senal)
filter_r <- as.data.frame(z)
return(filter_r)
  }

RSI_signals <- senal_rsi(oscilador= rsi,S_venta = Sobreventa,S_compra=Sobrecompra,inicio = MM_RSI)
head(RSI_signals)
```
#ADX 
```{r}
data(activo)
activo1 <- activo[,1:4]
dmi.adx <- ADX(activo[,1:4])
dmi.adx <- dmi.adx[,-3]
AADX <- cbind(dmi.adx,25)
#GRAFICO
dygraph(AADX, main= "ADX, +DMI & -DMI")%>%
  dyRangeSelector()
```
